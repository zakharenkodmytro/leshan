...
public class QueueModeLwM2mRequestSender implements LwM2mRequestSender2 {
    ...
    public <T extends LwM2mResponse> T send(final Registration destination, DownlinkRequest<T> request,
            LowerLayerSetter lowerLayerSetter, long timeout) throws InterruptedException {

        // If the client does not use Q-Mode, just send
        if (!destination.usesQueueMode()) {
            if (delegatedSender instanceof LwM2mRequestSender2) {
                return ((LwM2mRequestSender2) delegatedSender).send(destination, request, lowerLayerSetter, timeout);
            } else {
        ...
        try {
            T response = null;
            if (delegatedSender instanceof LwM2mRequestSender2) {
                response = ((LwM2mRequestSender2) delegatedSender).send(destination, request, lowerLayerSetter,
                        timeout);
            } else {
    ...
    public <T extends LwM2mResponse> void send(final Registration destination, DownlinkRequest<T> request,
            LowerLayerSetter lowerLayerSetter, long timeout, final ResponseCallback<T> responseCallback,
        ...
        if (delegatedSender instanceof LwM2mRequestSender2) {
            ((LwM2mRequestSender2) delegatedSender).send(destination, request, lowerLayerSetter, timeout,
